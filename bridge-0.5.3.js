...
async function startAgent(paneId, label) {
  ...
  if (launch2.command.length) {
    ...
    try {
      await run(["agent", "wait", paneId, "--until", "idle", "--timeout", "120000"], { timeout: 150000 });
    } catch {}
...
// packages/bridge/src/core.ts
var DROP_DIR = join5(process.env["LOCALAPPDATA"] || tmpdir3(), "browser-tweaks", "context");
...
function dropPath(payload) {
  ...
  return join5(DROP_DIR, `${stamp}-${slug}.md`);
}
async function send({ sessionId, newSession, cwd, payload }) {
  ...
  await mkdir4(DROP_DIR, { recursive: true });
  ...
  for (const element of elements) {
    ...
    await writeFile3(shot, Buffer.from(element.screenshot.split(",")[1] ?? "", "base64"));
    screenshots.set(element.ref, shot);
  }
  await writeFile3(file, renderContext(payload, screenshots), "utf8");
  let paneId;
  if (newSession) {
    if (!cwd)
...
import { mkdir as mkdir5, readFile as readFile3, readdir, rename, rm, stat, writeFile as writeFile4 } from "fs/promises";
import { join as join6 } from "path";
var distBase = () => process.env["BROWSER_TWEAKS_DIST"] || "https://wtiben.github.io/browser-tweaks-dist";
var versionsDir = () => join6(stateDir(), "versions");
var publicKeyFile = () => join6(stateDir(), "bin", "bridge-signing.pub");
var lastCheckFile = () => join6(stateDir(), "versions", ".last-check");
...
function isPublishedInstall(entry = process.argv[1]) {
  if (!entry)
    return false;
  return normalise(entry).startsWith(`${normalise(versionsDir())}/`);
}
async function dueForCheck() {
  try {
    const { mtimeMs } = await stat(lastCheckFile());
    return Date.now() - mtimeMs > CHECK_EVERY_MS;
  } catch {
    return true;
  }
}
...
async function maybeUpdate({ entry } = {}) {
  try {
    if (!isPublishedInstall(entry ?? process.argv[1]))
      return;
    if (!await dueForCheck())
      return;
    await mkdir5(versionsDir(), { recursive: true });
    await writeFile4(lastCheckFile(), new Date().toISOString(), "utf8");
    ...
    if (!verifySignature(bytes, release.signature, publicKey.trim()))
      return;
    const staging = join6(versionsDir(), `.staging-${release.version}`);
    await rm(staging, { recursive: true, force: true });
    await mkdir5(staging, { recursive: true });
    const staged = join6(staging, "bridge.js");
    await writeFile4(staged, bytes);
    if (!await selfTest(staged)) {
      await rm(staging, { recursive: true, force: true });
      return;
    }
    await rename(staging, join6(versionsDir(), release.version));
...
async function prune(installed) {
  ...
  for (const version of versions.slice(2)) {
    if (keep.has(version))
      continue;
    await rm(join6(versionsDir(), version), { recursive: true, force: true }).catch(() => {
      return;
    });
  }
}
